...
class PhotoAnalysisResponse(BaseModel):
    ...
    background_score: int = Field(
        ..., ge=1, le=10, description="Background quality (1-10)"
    )
    ...
    flags: List[str] = Field(
        default_factory=list, description="Issues or notable features"
    )
    ...
    def validate_overall_score(self) -> "PhotoAnalysisResponse":
        ...
        if abs(self.overall_score - expected) > 0.01:
            raise ValueError(
                f"overall_score {self.overall_score} does not equal average {expected:.2f}"
            )
