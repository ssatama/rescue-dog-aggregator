        ...
        with railway_session() as railway_conn:
            insert_sql = text(
                ...
                (id, name, animal_type, size, age_text, sex, breed,
                 ...
                 breed_confidence, breed_type, primary_breed, secondary_breed, breed_slug, images)
                VALUES
                (:id, :name, :animal_type, :size, :age_text, :sex, :breed,
                 :primary_image_url, :organization_id, :created_at, :updated_at,
                 :availability_confidence, :last_seen_at, :consecutive_scrapes_missing,
                    ...
                    batch_params.append(
                        {
                            ...
                            "breed_group": animal[21],
                            "breed_confidence": animal[22],
                            "breed_type": animal[23],
                            "primary_breed": animal[24],
                            "secondary_breed": animal[25],
                            "original_image_url": animal[26],
                            "last_seen_at": animal[27],
                            "consecutive_scrapes_missing": animal[28],
                            ...
                            "images": json.dumps(animal[38]) if animal[38] is not None else None,
                        }
                    )
...
def _build_organization_id_mapping(session) -> dict:
    ...
    try:
        ...
        result = session.execute(
            text(f"SELECT id, config_id FROM organizations WHERE config_id IN ({placeholders})"),
            params,
        )
        ...
        with get_pooled_connection() as local_conn:
            with local_conn.cursor() as cursor:
                ...
                for row in cursor.fetchall():
                    ...
                    config_id = row[1]
                    result = session.execute(
                        text("SELECT id FROM organizations WHERE config_id = :config_id"),
                        {"config_id": config_id},
                    )
