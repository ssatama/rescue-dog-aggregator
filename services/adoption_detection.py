...
class AdoptionDetectionService:
    ...
    ADOPTION_PATTERNS = [
        ...
        'found a home', 'has been adopted', 'gone home'
    ]
    
    RESERVED_PATTERNS = [
        'reserved', 'on hold', 'adoption pending',
        'application approved', 'pending adoption'
    ]
    
    AVAILABLE_PATTERNS = [
        'available', 'looking for a home', 'ready for adoption',
        'waiting for their forever home', 'could you give'
    ]
    ...
    def _create_extraction_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                "status": {
                    "type": "string",
                    "enum": ["adopted", "reserved", "available", "unknown"],
                    "description": "The detected adoption status"
                },
                "evidence": {
                    "type": "string",
                    "description": "The text that indicates the status"
                },
                "confidence": {
                    "type": "number",
                    "minimum": 0.0,
    ...
    def batch_check_adoptions(
        ...
        for dog_row in eligible_dogs:
            ...
            if not dry_run and not result.error:
                ...
                cursor.execute(
                    update_query,
                    (result.detected_status, result.checked_at, json.dumps(adoption_data), dog.id)
                )
