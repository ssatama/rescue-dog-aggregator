...
-- Then put the URL in PROD_RO_DATABASE_URL (local .env and, if the cloud can
-- reach Railway's TCP proxy, the cloud environment's variables):
--   postgresql://claude_ro:<password>@<RAILWAY_TCP_PROXY_DOMAIN>:<RAILWAY_TCP_PROXY_PORT>/railway?sslmode=require
--
-- The guard is SELECT-only grants. default_transaction_read_only is a second
-- layer only: a session can switch it off.
