...
import type { AgeQuickNavProps } from "@/types/pageComponents";

export default function AgeQuickNav({ currentSlug }: AgeQuickNavProps) {
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);

  const categories = getAgeCategoriesArray();
  const currentCategory = currentSlug
    ? AGE_CATEGORIES[currentSlug as keyof typeof AGE_CATEGORIES]
    : null;
