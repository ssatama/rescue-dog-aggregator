...
describe('Header Navigation Enhancement - Orange Theme', () => {
  ...
  describe('Active State Underline Indicators', () => {
    ...
    it('should show underline indicator for active Organizations link', () => {
      ...
      const underlineIndicator = screen.getByTestId('nav-underline-organizations');
      expect(underlineIndicator).toHaveClass('bg-orange-600');
    });
    ...
    it('should not show underline indicators for inactive links', () => {
      ...
      expect(screen.queryByTestId('nav-underline-dogs')).not.toBeInTheDocument();
      expect(screen.queryByTestId('nav-underline-organizations')).not.toBeInTheDocument();
      expect(screen.queryByTestId('nav-underline-about')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Mobile Menu Styling', () => {
    ...
    it('should use same orange theme in mobile menu', () => {
      ...
      const mobileFindDogsLink = screen.getAllByRole('link').find(link => 
        link.textContent === 'Find Dogs' && link.className.includes('block')
      );
