...
describe("BreedsHeroSection", () => {
  ...
  it("renders hero section with title and subtitle", () => {
    render(
      <BreedsHeroSection
        mixedBreedData={mockMixedBreedData}
        totalDogs={2717}
      />,
    );
    ...
    expect(
      screen.getByText(/one-of-a-kind mixed breed companions/i),
    ).toBeInTheDocument();
  });
  ...
  it("displays sample dog cards with images", () => {
    ...
    expect(lunaImage).toHaveAttribute(
      "src",
      expect.stringContaining("luna.jpg"),
    );
  ...
  it("handles missing sample dogs gracefully", () => {
    ...
    render(
      <BreedsHeroSection mixedBreedData={dataWithoutDogs} totalDogs={2717} />,
    );
