 ...
 * - Optional clickable stats with href
 */
export default function MobileStats({
  ...
  const StatContent = ({ stat }: { stat: Stat }) => (
    <div
      className={`text-center flex-1 ${stat.href ? "cursor-pointer" : ""}`}
      data-testid={`stat-${stat.label.toLowerCase()}`}
    >
      <div className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50 tracking-tight">
        {stat.value}
      </div>
      <div className={`text-[11px] font-medium uppercase tracking-wider mt-0.5 ${stat.href ? "text-orange-600 dark:text-orange-400" : "text-zinc-600 dark:text-zinc-400"}`}>
        {stat.label}
      </div>
    </div>
  );
    ...
    <div className="px-4 pb-4 sm:hidden">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 shadow-[0_1px_0_rgba(0,0,0,0.06),0_8px_20px_rgba(0,0,0,0.04)]">
        <div className="flex items-center justify-around">
          {stats.map((stat) =>
            stat.href ? (
              <Link key={stat.label} href={stat.href} className="flex-1">
                <StatContent stat={stat} />
              </Link>
            ) : (
              <StatContent key={stat.label} stat={stat} />
            )
          )}
        </div>
      </div>
