...
describe('MobileAvailableNow', () => {
  ...
  it('applies mobile-only visibility classes', () => {
    const { container } = render(<MobileAvailableNow dogs={mockDogs.slice(0, 4)} />);
  ...
  it('renders initial set of dogs with compact mode', () => {
    ...
    expect(screen.getByTestId('dog-card-1')).toHaveAttribute('data-compact', 'true');
  });
  ...
  it('hides Load More button when no more dogs', () => {
    ...
    expect(screen.queryByRole('button', { name: /load more dogs/i })).not.toBeInTheDocument();
  });
  ...
  it('handles empty dogs array gracefully', () => {
    ...
    expect(screen.getByText('Check back soon for new arrivals!')).toBeInTheDocument();
  });
  ...
  it('applies animation delays to dog cards', () => {
    ...
    expect(screen.getByTestId('dog-card-1')).toHaveAttribute('data-animation-delay', '0');
    expect(screen.getByTestId('dog-card-2')).toHaveAttribute('data-animation-delay', '1');
    expect(screen.getByTestId('dog-card-3')).toHaveAttribute('data-animation-delay', '2');
    expect(screen.getByTestId('dog-card-4')).toHaveAttribute('data-animation-delay', '3');
  });
  ...
  it('sets proper priority for first few dogs', () => {
    ...
    expect(screen.getByTestId('dog-card-1')).toHaveAttribute('data-priority', 'true');
    expect(screen.getByTestId('dog-card-2')).toHaveAttribute('data-priority', 'true');
    expect(screen.getByTestId('dog-card-3')).toHaveAttribute('data-priority', 'true');
    expect(screen.getByTestId('dog-card-4')).toHaveAttribute('data-priority', 'true');
    
    // Dogs after 4th should not have priority
    expect(screen.getByTestId('dog-card-5')).toHaveAttribute('data-priority', 'false');
    expect(screen.getByTestId('dog-card-6')).toHaveAttribute('data-priority', 'false');
  });
