 ...
 */
export default function FilterButton({ active = false, onClick, children, count = 0 }) {
  const handleClick = () => {
    if (onClick) {
      onClick();
    }
  ...
  return (
    ...
    >
      {children}
      {count > 0 && (
        <span className="ml-1.5 inline-flex items-center justify-center px-1.5 py-0.5 text-xs bg-orange-200 text-orange-800 rounded-full">
          {count}
        </span>
      )}
    </button>
