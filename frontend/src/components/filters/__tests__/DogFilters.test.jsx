...
describe("DogFilters Component", () => {
  ...
  describe("Age Filter", () => {
    test("displays all age options", async () => {
      ...
      expect(
        screen.getByRole("option", { name: "Young (1-3 years)" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: "Adult (3-8 years)" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: "Senior (8+ years)" }),
      ).toBeInTheDocument();
    });
  ...
  describe("Ships To Filter", () => {
    test("displays ships to options from availableShipsTo", async () => {
      ...
      expect(
        screen.getAllByRole("option", { name: "All Countries" }),
      ).toHaveLength(1);
      expect(
        screen.getByRole("option", { name: "Germany (DE)" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: "Netherlands (NL)" }),
      ).toBeInTheDocument();
    });
  ...
  describe("Performance", () => {
    ...
    test("handles large lists of available options efficiently", () => {
      const largeOptionsList = Array.from(
        { length: 1000 },
        (_, i) => `Breed ${i}`,
      );
