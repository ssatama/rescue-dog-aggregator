...
const DogsGrid = React.memo(function DogsGrid({
  ...
  if (loading) {
    ...
    return (
      <div
        ...
        className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 justify-center ${className} ${animationClass}`}
  ...
  return (
    <div
      ...
      className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 justify-center ${className}`}
      aria-label="Dogs available for adoption"
      {...props}
    >
      {dogs.map((dog, index) => {
