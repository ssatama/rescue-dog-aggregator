...
describe("CompatibilityIcons Component", () => {
  ...
  describe("Background Colors", () => {
    test("applies correct background colors for different values", () => {
      ...
      expect(screen.getByTestId("compatibility-icon-dogs")).toHaveClass("bg-green-100"); // yes
      expect(screen.getByTestId("compatibility-icon-cats")).toHaveClass("bg-red-100"); // no
      expect(screen.getByTestId("compatibility-icon-children")).toHaveClass("bg-yellow-100"); // maybe
    });

    test("applies gray background for unknown values", () => {
      render(<CompatibilityIcons profilerData={mockUnknownData} />);
      
      expect(screen.getByTestId("compatibility-icon-dogs")).toHaveClass("bg-gray-100");
      expect(screen.getByTestId("compatibility-icon-cats")).toHaveClass("bg-gray-100");
      expect(screen.getByTestId("compatibility-icon-children")).toHaveClass("bg-gray-100");
    });
  });
  ...
  describe("Confidence Score Filtering", () => {
    test("only shows icons with confidence > 0.5", () => {
      ...
      expect(screen.queryByTestId("dogs-compatibility")).not.toBeInTheDocument();
      expect(screen.queryByTestId("cats-compatibility")).not.toBeInTheDocument();
      expect(screen.queryByTestId("children-compatibility")).not.toBeInTheDocument();
    });
