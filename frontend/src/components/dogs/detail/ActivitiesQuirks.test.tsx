...
describe("ActivitiesQuirks Component", () => {
  ...
  test("renders activities with correct emojis when confidence > 0.5", () => {
    render(<ActivitiesQuirks profilerData={mockProfileDataWithHighConfidence} />);
  ...
  test("renders unique quirk when confidence > 0.5", () => {
    ...
    expect(screen.getByText("Loves to carry toys in his mouth everywhere")).toBeInTheDocument();
  });

  test("does not render activities when confidence <= 0.5", () => {
    render(<ActivitiesQuirks profilerData={mockProfileDataWithLowActivityConfidence} />);
  ...
  test("does not render quirk when confidence <= 0.5", () => {
    render(<ActivitiesQuirks profilerData={mockProfileDataWithLowQuirkConfidence} />);
  ...
  test("applies correct emoji mapping for different activities", () => {
    const dataWithVariousActivities: DogProfilerData = {
      favorite_activities: ["running", "zooming", "swimming", "playing", "toys", "fetch", "cuddling", "snuggling", "walking", "rolling", "sleeping"],
      confidence_scores: {
        favorite_activities: 0.9
      }
    };
