...
describe("DogDescription", () => {
  ...
  describe("Short Description Enhancement (<50 characters)", () => {
    it("should enhance short descriptions with contact prompt", () => {
      ...
      expect(
        screen.getByText(/Want to learn more about Buddy/),
      ).toBeInTheDocument();
      expect(screen.getByText(/Contact Happy Paws Rescue/)).toBeInTheDocument();
      expect(
        screen.getByText(/personality, needs, and adoption requirements/),
      ).toBeInTheDocument();
    });
    ...
    it("should handle very short descriptions properly", () => {
      ...
      render(
        <DogDescription {...mockProps} description={veryShortDescription} />,
      );
    ...
    it("should mark enhanced content for accessibility", () => {
      render(<DogDescription {...mockProps} description="Ready to fly!" />);

      const enhancedSection = screen.getByTestId("enhanced-description");
      expect(enhancedSection).toBeInTheDocument();
      expect(enhancedSection).toHaveAttribute(
        "aria-label",
        "Additional information about Buddy",
      );
    });
  ...
  describe("Empty Description Fallback", () => {
    it("should show engaging fallback when no description provided", () => {
      ...
      expect(
        screen.getByText(/Buddy is looking for a loving forever home/),
      ).toBeInTheDocument();
      expect(screen.getByText(/Contact Happy Paws Rescue/)).toBeInTheDocument();
      expect(
        screen.getByText(/wonderful dog's personality, needs/),
      ).toBeInTheDocument();
      expect(screen.getByText(/perfect home/)).toBeInTheDocument();
    });

    it("should show fallback for null description", () => {
      render(<DogDescription {...mockProps} description={null} />);
    ...
    it("should show fallback for undefined description", () => {
      render(<DogDescription {...mockProps} description={undefined} />);
    ...
    it("should mark fallback content for accessibility", () => {
      ...
      expect(fallbackContent).toHaveAttribute(
        "aria-label",
        "Default information about Buddy",
      );
    });
  });
  ...
  describe("Normal Description Functionality (>50 characters)", () => {
    const longDescription =
      "This is a wonderful dog with an amazing personality. He loves to play, is great with kids, and enjoys long walks in the park. He would make a perfect addition to any family looking for a loyal and loving companion. This dog has been well-trained and is ready for a new home. He is very social and gets along well with other dogs and cats. The perfect family pet that will bring joy and happiness to your home for many years to come.";

    it("should show full description when under read-more threshold", () => {
      const mediumDescription =
        "This is a medium length description that is longer than 50 characters but under 200.";

      render(<DogDescription {...mockProps} description={mediumDescription} />);
  ...
  describe("Typography and Styling", () => {
    it("should apply improved typography classes", () => {
      render(
        <DogDescription {...mockProps} description="Some description text" />,
      );
  ...
  describe("Edge Cases and Error Handling", () => {
    it("should handle missing dog name gracefully", () => {
      render(<DogDescription {...mockProps} dogName="" description="" />);
    ...
    it("should handle missing organization name gracefully", () => {
      render(
        <DogDescription {...mockProps} organizationName="" description="" />,
      );
    ...
    it("should sanitize HTML content properly", () => {
      ...
      render(<DogDescription {...mockProps} description={htmlDescription} />);
  ...
  describe("Performance and Accessibility", () => {
    it("should have proper ARIA attributes", () => {
      render(<DogDescription {...mockProps} description="Some description" />);
    ...
    it("should support keyboard navigation for read more button", () => {
      const veryLongDescription =
        "This is a very long description that should trigger the read more functionality and needs to be longer than 200 characters to properly test the truncation and expansion behavior. This text continues to make sure we definitely exceed the 200 character limit for triggering the read more button. Adding even more text to ensure this works properly in all test environments.";

      render(
        <DogDescription {...mockProps} description={veryLongDescription} />,
      );
    ...
    it("should maintain focus management during expansion", () => {
      ...
      expect(document.activeElement).toBe(
        screen.getByTestId("read-more-button"),
      );
    });
  });
});
