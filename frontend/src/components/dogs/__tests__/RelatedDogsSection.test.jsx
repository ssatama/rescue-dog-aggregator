...
describe('RelatedDogsSection', () => {
  ...
  describe('Success State with Related Dogs', () => {
    ...
    it('should render "View all available dogs" link', async () => {
      ...
      await waitFor(() => {
        ...
        expect(viewAllLink.closest('a')).toHaveAttribute('href', '/dogs?organization_id=456');
      });
    });
  ...
  describe('Empty State', () => {
    ...
    it('should show empty state message when no related dogs', async () => {
      ...
      // Assert
      await waitFor(() => {
        expect(screen.getByText('No other dogs available from this rescue')).toBeInTheDocument();
      });
    });
    ...
    it('should maintain consistent styling in empty state', async () => {
      ...
      // Assert
      await waitFor(() => {
        expect(screen.getByTestId('related-dogs-empty-state')).toBeInTheDocument();
        expect(screen.getByTestId('related-dogs-empty-state')).toHaveClass('text-center');
        expect(screen.getByTestId('related-dogs-empty-state')).toHaveClass('py-8');
      });
    });
  });
  ...
  describe('Error State', () => {
    ...
    it('should show error message when API fails', async () => {
      ...
      // Assert
      await waitFor(() => {
        expect(screen.getByText(/Unable to load related dogs/)).toBeInTheDocument();
      });
    });
  });
