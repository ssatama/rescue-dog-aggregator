 ...
 */
const MobileCarousel = ({ children, onSlideChange, testId = "dog-carousel" }) => {
  ...
  const scrollToSlide = useCallback((slideIndex) => {
    if (carouselRef.current) {
      const slideWidth = carouselRef.current.querySelector('.flex-shrink-0')?.offsetWidth || 0;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const slideWidth = carousel.querySelector('.flex-shrink-0')?.offsetWidth || 0;
      const gap = 16;
