...
const MobileCarousel: React.FC<MobileCarouselProps> = ({ 
  ...
  const scrollToSlide = useCallback((slideIndex: number) => {
    if (carouselRef.current) {
      const slideElement = carouselRef.current.querySelector('.flex-shrink-0') as HTMLElement;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const slideElement = carousel.querySelector('.flex-shrink-0') as HTMLElement;
      const slideWidth = slideElement?.offsetWidth || 0;
      const gap = 16;
