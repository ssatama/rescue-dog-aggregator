...
describe('EmptyState', () => {
  ...
  describe('Variant: No Dogs Filtered', () => {
    it('renders correct content for noDogsFiltered variant', () => {
      ...
      render(<EmptyState variant="noDogsFiltered" onClearFilters={mockClearFilters} />);
      
      expect(screen.getByText('No dogs match your filters')).toBeInTheDocument();
      expect(screen.getByText(/Try adjusting your search criteria/)).toBeInTheDocument();
      
      const clearFiltersButton = screen.getByText('Clear All Filters & Start Fresh');
    ...
    it('renders filter icon for noDogsFiltered variant', () => {
      ...
      expect(icon.querySelector('path')).toHaveAttribute('d', expect.stringContaining('M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.414A1 1 0 013 6.707V4z'));
    });
  });
  ...
  describe('Variant: No Dogs Organization', () => {
    it('renders correct content for noDogsOrganization variant', () => {
      ...
      render(<EmptyState variant="noDogsOrganization" onBrowseOrganizations={mockBrowseOrganizations} />);
      
      expect(screen.getByText('No dogs available right now')).toBeInTheDocument();
      expect(screen.getByText(/This organization doesn't have any dogs/)).toBeInTheDocument();
    ...
    it('renders heart icon for noDogsOrganization variant', () => {
      ...
      const icon = screen.getByTestId('empty-state-icon');
      expect(icon).toBeInTheDocument();
      expect(icon.querySelector('path')).toHaveAttribute('d', expect.stringContaining('M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z'));
    });
  });
  ...
  describe('Variant: No Organizations', () => {
    it('renders correct content for noOrganizations variant', () => {
      ...
      expect(screen.getByText(/We couldn't find any rescue organizations/)).toBeInTheDocument();
    ...
    it('renders building icon for noOrganizations variant', () => {
      ...
      const icon = screen.getByTestId('empty-state-icon');
      expect(icon).toBeInTheDocument();
      expect(icon.querySelector('path')).toHaveAttribute('d', expect.stringContaining('M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z'));
    });
  });
  ...
  describe('Styling and Layout', () => {
    ...
    it('renders icon with correct styling', () => {
      ...
      const icon = screen.getByTestId('empty-state-icon');
      expect(icon).toHaveClass('h-16', 'w-16', 'mx-auto', 'text-orange-400', 'mb-2');
    });
  ...
  describe('Accessibility', () => {
    it('has proper ARIA attributes', () => {
      ...
      expect(emptyState).toHaveAttribute('aria-label', 'Empty state: Test Title');
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('handles missing callback functions gracefully', () => {
      ...
      expect(screen.queryByText('Clear All Filters & Start Fresh')).not.toBeInTheDocument();
    });

    it('renders without action button when no callback provided', () => {
      render(<EmptyState variant="noDogsFiltered" onClearFilters={undefined} />);
