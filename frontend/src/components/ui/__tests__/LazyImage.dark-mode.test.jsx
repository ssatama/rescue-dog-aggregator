...
jest.mock('../ImagePlaceholder', () => {
  return function MockImagePlaceholder({ className }) {
    return <div className={className} data-testid="mock-image-placeholder">Mock Placeholder</div>;
  };
});
...
describe('LazyImage Dark Mode', () => {
  ...
  describe('Error State Dark Mode', () => {
    ...
    test('error state maintains accessibility in dark mode', () => {
      ...
      expect(errorPlaceholder).toHaveAttribute('aria-label', 'Profile picture - Failed to load');
    });
  });
  ...
  describe('Accessibility in Dark Mode', () => {
    test('maintains proper ARIA labels in dark mode', () => {
      ...
      expect(placeholder).toHaveAttribute('aria-label', 'Accessible test image');
    });
    ...
    test('loaded image maintains accessibility attributes', () => {
      ...
      if (images.length > 0) {
        expect(images[images.length - 1]).toHaveAttribute('alt', 'Final loaded image');
      }
