...
export const FallbackImage: React.FC<FallbackImageProps> = ({
  ...
  useEffect(() => {
    ...
    setFallbackLevel(0);
    setIsR2Image(srcString.includes(R2_CUSTOM_DOMAIN) || srcString.includes("r2.cloudflarestorage.com"));
  }, [src]);

  const getR2FallbackUrl = useCallback((originalUrl: string, level: number): string => {
    try {
      const url = new URL(originalUrl);

      // If it's already an R2 URL
      if (url.hostname === R2_CUSTOM_DOMAIN || url.hostname.includes("r2.cloudflarestorage.com")) {
        if (level === 1) {
  ...
  const handleError = useCallback((error: any) => {
    console.warn(`Image failed to load (level ${fallbackLevel}):`, imageSrc, error);
  ...
  }, [fallbackLevel, imageSrc, src, fallbackSrc, customOnError, getR2FallbackUrl]);
