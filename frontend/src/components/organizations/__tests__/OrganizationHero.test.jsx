...
jest.mock("../../ui/CountryFlag", () => {
  return function MockedCountryFlag({ countryCode, countryName, size }) {
    return (
      <span data-testid={`flag-${countryCode}`} data-size={size}>
        {countryCode}
      </span>
    );
  };
...
describe("OrganizationHero", () => {
  const mockOrganization = {
    ...
    name: "Pets in Turkey",
    description:
      "Dedicated to rescuing street dogs in Turkey and finding them loving homes across Europe. Every dog deserves a second chance at happiness.",
    country: "TR",
    city: "Izmir",
  ...
  describe("Basic Rendering", () => {
    ...
    test("renders organization description", () => {
      ...
      expect(
        screen.getByText(/Dedicated to rescuing street dogs/),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Logo Display", () => {
    ...
    test("renders initials fallback when no logo_url", () => {
      ...
      expect(initialsElement.parentElement).toHaveClass(
        "w-20",
        "h-20",
        "md:w-32",
        "md:h-32",
    ...
    test("displays smaller logo on mobile", () => {
      ...
      const logoContainer = screen.getByAltText(
        "Pets in Turkey logo",
      ).parentElement;
  ...
  describe("Breadcrumb Navigation", () => {
    ...
    test("shows current organization name in breadcrumb", () => {
      ...
      const orgNameInBreadcrumb = screen
        .getAllByText("Pets in Turkey")
        .find((el) => breadcrumbNav.contains(el));
  ...
  describe("Call to Action", () => {
    ...
    test("handles missing website_url gracefully", () => {
      ...
      expect(
        screen.queryByText("Visit Original Website"),
  ...
  describe("Social Media Links", () => {
    ...
    test("hides social media section when no links available", () => {
      ...
      expect(
        screen.queryByTestId("social-media-section"),
      ).not.toBeInTheDocument();
    });
  });
