...
jest.mock('../../ui/SocialMediaLinks', () => {
  return function MockedSocialMediaLinks({ socialMedia, className, size }) {
    return (
      <div data-testid="social-media-links" className={className} data-size={size}>
        Social Links
      </div>
    );
  };
...
describe('OrganizationCard Size Variants', () => {
  ...
  describe('Size Prop Acceptance', () => {
    ...
    test('defaults to size="large" when no size prop provided', () => {
      ...
      const logos = screen.getAllByTestId('lazy-image');
      expect(logos[0]).toHaveClass('w-16', 'h-16'); // 64px (large)
    });
  });
  ...
  describe('Logo Size Scaling', () => {
    test('renders 48px logo for size="small"', () => {
      render(<OrganizationCard organization={mockOrganization} size="small" />);
      const logo = screen.getAllByTestId('lazy-image')[0];
      expect(logo).toHaveClass('w-12', 'h-12'); // 48px
    });

    test('renders 56px logo for size="medium"', () => {
      render(<OrganizationCard organization={mockOrganization} size="medium" />);
      const logo = screen.getAllByTestId('lazy-image')[0];
      expect(logo).toHaveClass('w-14', 'h-14'); // 56px
    });

    test('renders 64px logo for size="large"', () => {
      render(<OrganizationCard organization={mockOrganization} size="large" />);
      const logo = screen.getAllByTestId('lazy-image')[0];
      expect(logo).toHaveClass('w-16', 'h-16'); // 64px
    });
    ...
    test('initials placeholder scales with size', () => {
      ...
      // Small
      const { rerender } = render(<OrganizationCard organization={orgWithoutLogo} size="small" />);
      ...
      // Medium
      rerender(<OrganizationCard organization={orgWithoutLogo} size="medium" />);
  ...
  describe('Padding and Spacing Adjustments', () => {
    test('card header padding scales with size', () => {
      const { container, rerender } = render(<OrganizationCard organization={mockOrganization} size="small" />);
  ...
  describe('Dog Preview Thumbnails Scaling', () => {
    test('dog thumbnail size scales with card size', () => {
      ...
      dogThumbnails.forEach(img => {
        expect(img).toHaveClass('w-10', 'h-10'); // 40px for small
      });
      
      rerender(<OrganizationCard organization={mockOrganization} size="medium" />);
      dogThumbnails = screen.getAllByTestId('lazy-image').slice(1);
      dogThumbnails.forEach(img => {
        expect(img).toHaveClass('w-11', 'h-11'); // 44px for medium
      });
      
      rerender(<OrganizationCard organization={mockOrganization} size="large" />);
      dogThumbnails = screen.getAllByTestId('lazy-image').slice(1);
      dogThumbnails.forEach(img => {
        expect(img).toHaveClass('w-12', 'h-12'); // 48px for large
      });
    });
  ...
  describe('Social Media Links Scaling', () => {
    test('social media icon size prop changes with card size', () => {
      const { rerender } = render(<OrganizationCard organization={mockOrganization} size="small" />);
      expect(screen.getByTestId('social-media-links')).toHaveAttribute('data-size', 'xs');
      
      rerender(<OrganizationCard organization={mockOrganization} size="medium" />);
      expect(screen.getByTestId('social-media-links')).toHaveAttribute('data-size', 'sm');
    ...
    test('small cards maintain 3-column grid on desktop', () => {
      const { container } = render(
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <OrganizationCard organization={{...mockOrganization, id: 2}} size="small" />
          <OrganizationCard organization={{...mockOrganization, id: 3}} size="small" />
        </div>
      );
    ...
    test('all sizes work in responsive layouts', () => {
      const { container } = render(
        <div className="space-y-4">
          <OrganizationCard organization={mockOrganization} size="small" />
          <OrganizationCard organization={{...mockOrganization, id: 2}} size="medium" />
          <OrganizationCard organization={{...mockOrganization, id: 3}} size="large" />
        </div>
      );
      
      expect(container.querySelectorAll('[data-testid="lazy-image"]').length).toBeGreaterThanOrEqual(3); // At least one logo per card
    });
  });
  ...
  describe('Accessibility at Different Sizes', () => {
    ...
    test('all interactive elements remain keyboard accessible', () => {
      ['small', 'medium', 'large'].forEach(size => {
        const { container } = render(<OrganizationCard organization={mockOrganization} size={size} />);
        const mainCard = container.querySelector('[role="button"][tabIndex="0"]');
