...
jest.mock('../../ui/LoadingSkeleton', () => ({
  TrustStatsSkeleton: () => <div data-testid="trust-stats-skeleton">Loading...</div>
}));
...
describe('TrustSection Homepage Integration', () => {
  ...
  describe('Error Handling', () => {
    ...
    test('handles empty organizations array', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryAllByTestId('organization-card-mock')).toHaveLength(0);
      });
    });
  });
  ...
  describe('Loading State', () => {
    ...
    test('removes skeleton after data loads', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('trust-stats-skeleton')).not.toBeInTheDocument();
      });
    });
  });
});