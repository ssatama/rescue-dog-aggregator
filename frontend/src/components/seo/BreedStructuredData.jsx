...
export default function BreedStructuredData({ breedData, dogs, pageType = 'detail' }) {
  if (!breedData) return null;

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.rescuedogs.me';
  ...
  const articleSchema = breedData.description ? {
    ...
    'description': breedData.description?.tagline || breedData.description?.overview || `Learn about ${breedData.primary_breed} dogs available for adoption`,
    'author': {
      '@type': 'Organization',
      'name': 'Rescue Dogs',
      'url': baseUrl
  ...
  const faqSchema = {
    ...
    'mainEntity': [
      ...
      },
      {
        ...
        'acceptedAnswer': {
          '@type': 'Answer',
          'text': breedData.description?.temperament || `${breedData.primary_breed || 'Mixed breed'} dogs have unique personalities. Each dog's temperament is assessed by rescue organizations to help match them with suitable homes.`
        }
      },
      {
        '@type': 'Question',
        ...
        'acceptedAnswer': {
          '@type': 'Answer',
          'text': breedData.description?.family || `Many ${breedData.primary_breed || 'mixed breed'} dogs can be excellent with children when properly socialized. Each dog's compatibility with children is evaluated individually by rescue organizations.`
        }
      }
    ]
  };
  ...
  const collectionSchema = pageType === 'hub' ? {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    'name': 'Dog Breeds Available for Adoption',
    'description': 'Browse rescue dogs by breed with real-time availability from verified organizations',
    'url': `${baseUrl}/breeds`,
    'numberOfItems': breedData.unique_breeds || 0,
    'hasPart': breedData.qualifying_breeds?.map(breed => ({
      '@type': 'WebPage',
  ...
  const itemListSchema = dogs && dogs.length > 0 ? {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    ...
    'itemListElement': dogs.map((dog, index) => ({
      ...
      'item': {
        '@type': 'Product',
        'name': dog.name,
        'description': dog.properties?.description || `Meet ${dog.name}, a ${dog.breed || 'mixed breed'} dog available for adoption`,
