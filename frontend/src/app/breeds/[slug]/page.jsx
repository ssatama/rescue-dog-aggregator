...
import { getBreedBySlug, getAnimals, getBreedStats } from '@/services/serverAnimalsService';
  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        images: breedData.topDogs?.slice(0, 4).map(d => ({
      ...
      twitter: {
        ...
        title: `${breedData.count} ${breedData.primary_breed} Dogs Need Homes`,
        description: `Browse ${breedData.primary_breed} rescue dogs with personality profiles and real-time availability.`,
        images: breedData.topDogs?.[0]?.primary_image_url ? [breedData.topDogs[0].primary_image_url] : [],
      },
    };
  } catch (error) {
...
export async function generateStaticParams() {
  try {
    ...
    return breedStats.qualifying_breeds?.map(breed => ({
...
export default async function BreedDetailPage(props) {
  try {
    ...
    const initialDogs = initialDogsResponse?.results || initialDogsResponse || [];
