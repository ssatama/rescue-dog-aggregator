...
export default function BreedDetailClient({ initialBreedData, initialDogs, initialParams }) {
  ...
  const loadDogs = useCallback(async (newOffset = 0, append = false) => {
    setLoading(true);
    try {
      const filters = {
        ...activeFilters,
  ...
  useEffect(() => {
    ...
    router.replace(`/breeds/${breedData.breed_slug}${queryString ? `?${queryString}` : ''}`, { scroll: false });
  }, [activeFilters]);
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 dark:text-gray-400">First-time OK</span>
                      <span className="font-semibold">{breedData.experience_distribution.first_time_ok || 0} dogs</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 dark:text-gray-400">Some Experience</span>
                      <span className="font-semibold">{breedData.experience_distribution.some_experience || 0} dogs</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 dark:text-gray-400">Experienced</span>
                      <span className="font-semibold">{breedData.experience_distribution.experienced || 0} dogs</span>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex gap-2 overflow-x-auto pb-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() => handleFilterChange('goodWithCats', !activeFilters.goodWithCats)}
                className="flex-shrink-0"
              >
                😺 Cat Friendly
              </Button>
              <Button
                ...
                size="sm"
                onClick={() => handleFilterChange('goodWithDogs', !activeFilters.goodWithDogs)}
                className="flex-shrink-0"
              >
                🐕 Dog Friendly
              </Button>
