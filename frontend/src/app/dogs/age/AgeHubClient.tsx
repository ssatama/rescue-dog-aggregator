...
import type { AgeCategory, AgeHubClientProps } from "@/types/pageComponents";
import { AGE_CATEGORIES, getAgeCategoriesArray } from "@/utils/ageData";

function AgeCategoryCard({ category }: { category: AgeCategory & { count: number } }) {
  const isPuppies = category.slug === "puppies";
  const Icon = isPuppies ? Sparkles : Heart;

  return (
    <Link href={`/dogs/${category.slug}`}>
      <Card className="group relative overflow-hidden h-full hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-2 border-transparent hover:border-orange-300 dark:hover:border-orange-600">
        <div
          className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${category.gradient}`}
        />
        ...
        <div className="p-6">
          ...
          <div className="flex items-center text-orange-500 text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity">
            {isPuppies ? "View puppies" : "View senior dogs"}
        ...
        <div className="relative max-w-5xl mx-auto text-center">
          <div className="animate-fade-in">
            ...
            <p className="text-lg md:text-xl opacity-95 max-w-2xl mx-auto mb-8">
              The youngest and oldest rescue dogs often wait the longest. These
              special life stages deserve extra love.
            </p>
          </div>

          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {puppies && (
              <Link href={`/dogs/${puppies.slug}`}>
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {categoriesWithStats.map((category, index) => (
              ...
              >
                <AgeCategoryCard category={category} />
              </div>
            ))}
          </div>
