...
export default function DogsPageClientSimplified({ 
  ...
  const validateOrganizationId = (orgId) => {
    ...
    const isValidOrg = organizations.some(org => 
      org.id?.toString() === orgId || org.id === parseInt(orgId, 10)
    );
  ...
  const filters = {
    ...
    organizationFilter: validateOrganizationId(searchParams.get("organization_id")),
  ...
  useEffect(() => {
    if (filters.availableCountryFilter && filters.availableCountryFilter !== "Any country") {
      getAvailableRegions(filters.availableCountryFilter).then(regions => {
