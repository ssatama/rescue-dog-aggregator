...
export default function DogsPageClient() {
  ...
  useEffect(() => {
    getOrganizations()
      .then(orgs => {
        React.startTransition(() => {
          setOrganizations([{ id: null, name: "Any organization" }, ...(Array.isArray(orgs) ? orgs : [])]);
        });
      })
  ...
  const renderActiveFilters = () => {
    ...
    return (
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {filters.map(filter => (
          <Badge key={filter.type} data-testid="active-filter-badge" variant="secondary" className="flex items-center gap-1">
            {filter.label}
            <button
        ...
        <Button variant="link" size="sm" onClick={resetFilters} className="text-orange-600 hover:text-orange-700 p-0 h-auto font-medium enhanced-focus-link" data-testid="clear-all-filters-button">
          Clear All
        </Button>
      </div>
    );
