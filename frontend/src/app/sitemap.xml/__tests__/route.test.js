...
describe('Dynamic Sitemap Route', () => {
  ...
  test('should generate valid XML sitemap with all content', async () => {
    ...
    expect(response.body).toContain('<loc>https://rescuedogs.me/dogs/buddy-mixed-breed-1</loc>');
    expect(response.body).toContain('<loc>https://rescuedogs.me/dogs/luna-labrador-2</loc>');
    
    // Should include organization pages
    expect(response.body).toContain('<loc>https://rescuedogs.me/organizations/happy-paws-rescue-1</loc>');
    expect(response.body).toContain('<loc>https://rescuedogs.me/organizations/city-shelter-2</loc>');
    
    // Should include static pages
    expect(response.body).toContain('<loc>https://rescuedogs.me/dogs</loc>');
    expect(response.body).toContain('<loc>https://rescuedogs.me/organizations</loc>');
  ...
  test('should use custom base URL from environment', async () => {
    ...
    expect(response.body).toContain('<loc>https://staging.rescuedogs.me/</loc>');
    expect(response.body).toContain('<loc>https://staging.rescuedogs.me/dogs/buddy-mixed-breed-1</loc>');
  });
  ...
  test('should include lastmod dates when available', async () => {
    ...
    expect(response.body).toContain('<lastmod>2025-07-14T08:58:28+00:00</lastmod>');
    expect(response.body).toContain('<lastmod>2025-07-13T10:30:15+00:00</lastmod>');
  });
  ...
  test('should filter out invalid slugs', async () => {
    ...
    expect(response.body).toContain('<loc>https://rescuedogs.me/dogs/valid-dog-1</loc>');
    expect(response.body).not.toContain('<loc>https://rescuedogs.me/dogs/null</loc>');
    expect(response.body).not.toContain('<loc>https://rescuedogs.me/dogs/</loc>');
  });
  ...
  test('should include priority and changefreq for SEO optimization', async () => {
    ...
    expect(response.body).toMatch(/<url>[\s\S]*?<loc>https:\/\/rescuedogs\.me\/<\/loc>[\s\S]*?<priority>1<\/priority>[\s\S]*?<\/url>/);
    
    // Dog pages should have appropriate priority
    expect(response.body).toMatch(/<url>[\s\S]*?<loc>https:\/\/rescuedogs\.me\/dogs\/buddy-mixed-breed-1<\/loc>[\s\S]*?<priority>0\.8<\/priority>[\s\S]*?<\/url>/);
