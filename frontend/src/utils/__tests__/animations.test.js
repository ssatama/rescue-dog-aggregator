...
describe('Animation Utilities', () => {
  ...
  describe('useHoverAnimation', () => {
    ...
    it('should apply correct transform styles based on hover state', () => {
      ...
      // Initial state
      expect(result.current.hoverProps.style.transform).toBe('scale(1) translateY(0px)');
      ...
      expect(result.current.hoverProps.style.transform).toBe('scale(1.05) translateY(-8px)');
  ...
  describe('Accessibility', () => {
    it('should respect prefers-reduced-motion in all hooks', () => {
      ...
      const { result: staggerResult } = renderHook(() => useStaggerAnimation(3));
  ...
  describe('Performance', () => {
    ...
    it('should use requestAnimationFrame for smooth animations', () => {
      ...
      expect(result.current.hoverProps.style.transition).toContain('cubic-bezier');
    });
  });
});
