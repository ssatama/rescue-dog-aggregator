...
describe('dogHelpers', () => {
  ...
  describe('formatGender', () => {
    it('formats male gender with icon', () => {
      expect(formatGender({ sex: 'Male' })).toEqual({ text: 'Male', icon: '♂️' });
      expect(formatGender({ sex: 'male' })).toEqual({ text: 'Male', icon: '♂️' });
    ...
    it('formats female gender with icon', () => {
      expect(formatGender({ sex: 'Female' })).toEqual({ text: 'Female', icon: '♀️' });
      expect(formatGender({ sex: 'female' })).toEqual({ text: 'Female', icon: '♀️' });
      expect(formatGender({ sex: 'F' })).toEqual({ text: 'Female', icon: '♀️' });
      expect(formatGender({ sex: 'f' })).toEqual({ text: 'Female', icon: '♀️' });
    });

    it('handles unknown gender', () => {
      expect(formatGender({ sex: 'Unknown' })).toEqual({ text: 'Unknown', icon: '❓' });
  ...
  describe('getOrganizationName', () => {
    ...
    it('returns fallback when organization not available', () => {
      ...
      expect(getOrganizationName({ organization: {} })).toBe('Unknown Organization');
    });
  });
