...
const realR2Domain = process.env.NEXT_PUBLIC_R2_CUSTOM_DOMAIN || 'images.rescuedogs.me';
...
describe('Error Handling', () => {
  describe('handleImageError', () => {
    ...
    it('should try R2 URL without transformations if transformation fails', () => {
      ...
      expect(mockEvent.target.src).toBe(`https://${realR2Domain}/animals/test-org/sample.jpg`);
    });
...
describe('New Consolidated Image Functions', () => {
  ...
  describe('validateImageUrl', () => {
    it('should validate secure R2 URLs', () => {
      ...
      expect(validateImageUrl(`https://${realR2Domain}/valid/path/image.jpg`)).toBe(true);
    });

    it('should reject URLs with path traversal attempts', () => {
      expect(validateImageUrl(`https://${realR2Domain}/../malicious/path.jpg`)).toBe(false);
      expect(validateImageUrl(`https://${realR2Domain}/folder/../../../secret.jpg`)).toBe(false);
      expect(validateImageUrl(`https://${realR2Domain}/./file.jpg`)).toBe(false);
    });
  ...
  describe('buildSecureCloudflareUrl', () => {
    ...
    it('should reject invalid URLs for security', () => {
      ...
      expect(() => buildSecureCloudflareUrl(maliciousUrl, params)).toThrow('Invalid image path');
    });

    it('should sanitize transformation parameters', () => {
      const maliciousParams = 'w=400,h=300,fit=cover,quality=auto;DROP TABLE users;';
      
      expect(() => buildSecureCloudflareUrl(r2Url, maliciousParams)).toThrow('Invalid transformation parameters');
    });
...
describe('Performance and Caching Tests', () => {
  ...
  describe('Parameter Consistency', () => {
    it('should generate consistent parameters for same preset to improve CDN caching', () => {
      const result1 = getOptimizedImage(r2Url, 'catalog');
      const result2 = getOptimizedImage(`https://${realR2Domain}/other/image.jpg`, 'catalog');
