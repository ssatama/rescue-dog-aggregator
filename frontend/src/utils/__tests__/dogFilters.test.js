...
describe('dogFilters', () => {
  ...
  describe('filterByShipsTo', () => {
    test('filters dogs by organization ships_to countries', () => {
      ...
      expect(result.map(d => d.name)).toEqual(['Buddy', 'Luna', 'Charlie', 'Bella']);
    });
  ...
  describe('sortDogs', () => {
    ...
    test('sorts by name A-Z', () => {
      ...
      expect(result.map(d => d.name)).toEqual(['Bella', 'Buddy', 'Charlie', 'Luna', 'Max']);
    });

    test('sorts by name Z-A', () => {
      const result = sortDogs(mockDogs, 'name-desc');
      expect(result.map(d => d.name)).toEqual(['Max', 'Luna', 'Charlie', 'Buddy', 'Bella']);
    });
