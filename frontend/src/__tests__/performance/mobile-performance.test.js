...
describe('Mobile Performance Tests', () => {
  ...
  describe('Touch Target Validation', () => {
    ...
    test('breadcrumb navigation is touch-friendly', async () => {
      ...
      const breadcrumbLinks = screen.getAllByRole('link').filter(link => 
        link.closest('nav[aria-label*="breadcrumb" i]')
      );
  ...
  describe('Scroll Performance', () => {
    test('smooth scrolling behavior is implemented', async () => {
      ...
      const sections = ['hero-image-container', 'metadata-cards', 'about-section'];
  ...
  describe('Mobile-Specific Features', () => {
    ...
    test('text is readable on mobile screens', async () => {
      ...
      headings.forEach(heading => {
        ...
        if (fontSize > 10) { // Only test if we get a reasonable font size from test environment
