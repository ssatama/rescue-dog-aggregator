...
jest.mock("../../components/dogs/DogsGrid", () => {
  const DogsGrid = ({ dogs, loading }) => (
    <div data-testid="dogs-grid">
      {loading ? "Loading..." : `Dogs: ${dogs.length}`}
    </div>
  );
...
describe("DogsPageClient with useFilterState Integration", () => {
  ...
  describe("Filter State Consolidation", () => {
    ...
    test("FAILING TEST: API parameters should use apiParams from useFilterState", async () => {
      const mockGetAnimals =
        require("../../services/animalsService").getAnimals;
      ...
      const lastCall =
        mockGetAnimals.mock.calls[mockGetAnimals.mock.calls.length - 1];
