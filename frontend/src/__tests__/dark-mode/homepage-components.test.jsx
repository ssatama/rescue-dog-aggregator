...
describe('Homepage Components - Dark Mode Support', () => {
  ...
  describe('HeroSection Component Dark Mode', () => {
    test('should use semantic text colors instead of hard-coded grays in dark mode', () => {
      ...
      const heroSection = document.querySelector('.hero-gradient').closest('section');
    ...
    test('should use semantic background for statistics cards in dark mode', async () => {
      ...
      getStatistics.mockResolvedValueOnce({
        ...
        countries: ['DE', 'US'],
        organizations: [
          { id: 1, name: 'Test Rescue', dog_count: 10 }
        ]
      });
      ...
      const allCards = document.querySelectorAll('[class*="bg-card"], [class*="bg-white"]');
    ...
    test('should handle loading states properly in dark mode', () => {
      ...
      loadingElements.forEach(element => {
        ...
        if (element.className.includes('from-') || element.className.includes('to-')) {
          expect(element.className).toMatch(/bg-muted|from-muted|to-muted|dark:/);
        }
      });
    });
    ...
    test('should maintain orange theme for call-to-action buttons in dark mode', () => {
      ...
      const ctaButtons = screen.getAllByRole('button').filter(button => 
        button.className.includes('bg-orange') || button.textContent.includes('Find')
      );

      ctaButtons.forEach(button => {
        // Orange theme should be preserved but may have dark mode variants
        expect(button.className).toMatch(/bg-orange|hover:bg-orange|dark:bg-orange/);
      });
    });
  });
  ...
  describe('TrustSection Component Dark Mode', () => {
    ...
    test('should use semantic colors for statistics cards in dark mode', async () => {
      ...
      const cardTexts = document.querySelectorAll('[class*="text-gray-900"], [class*="text-gray-600"]');
      cardTexts.forEach(text => {
        ...
        if (text.className.includes('text-')) {
          expect(text.className).toMatch(/text-foreground|text-muted-foreground|dark:text-/);
        }
      });
    });
    ...
    test('should maintain icon colors but ensure proper contrast in dark mode', async () => {
      ...
      const iconContainers = document.querySelectorAll('[class*="text-orange"], [class*="text-green"]');
  ...
  describe('DogSection Component Dark Mode', () => {
    test('should use semantic text colors for section headers in dark mode', async () => {
      ...
      headers.forEach(header => {
        if (header.textContent.includes('Featured') || header.textContent.includes('Dogs')) {
          // Should not use hard-coded gray colors
          ...
          if (header.className.includes('text-')) {
            expect(header.className).toMatch(/text-foreground|text-muted-foreground|dark:text-/);
          }
        }
      });
    });
    ...
    test('should handle empty state properly in dark mode', async () => {
      ...
      const emptyStateTexts = document.querySelectorAll('[class*="text-gray-500"]');
  ...
  describe('Homepage Components Integration in Dark Mode', () => {
    test('should work together cohesively in dark mode', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/helping rescue dogs find loving homes/i)).toBeInTheDocument();
        expect(screen.getByText(/featured rescue dogs/i)).toBeInTheDocument();
      });

      // All sections should be present and working
      const heroSection = document.querySelector('.hero-gradient').closest('section');
      const dogSection = screen.getByText(/featured rescue dogs/i).closest('section');
    ...
    test('should maintain consistent theming across all homepage sections', async () => {
      ...
      allTextElements.forEach(element => {
        ...
        if (element.className.includes('text-gray')) {
          expect(element.className).toMatch(/dark:text-|text-foreground|text-muted-foreground/);
        }
      });

      const allBgElements = homepage.querySelectorAll('[class*="bg-gray"], [class*="bg-white"]');
      
      allBgElements.forEach(element => {
        // All backgrounds should either have dark mode variants or use semantic tokens
        if (element.className.includes('bg-gray') || element.className.includes('bg-white')) {
          expect(element.className).toMatch(/dark:bg-|bg-background|bg-card|bg-muted/);
        }
      });
    });
  });
