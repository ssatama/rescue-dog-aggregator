...
jest.mock('../../components/ui/LazyImage', () => {
  return function MockLazyImage({ alt, className, placeholder, priority, enableProgressiveLoading, ...props }) {
...
const getMockOrganization = (overrides = {}) => {
  return {
    ...
    new_this_week: 3,
    recent_dogs: [
      { id: 1, name: "Buddy", primary_image_url: "https://example.com/dog1.jpg" },
      { id: 2, name: "Luna", primary_image_url: "https://example.com/dog2.jpg" },
...
describe('OrganizationCard Component - Dark Mode Support', () => {
  ...
  describe('Logo Background Dark Mode', () => {
    test('should use semantic background for logo placeholder instead of hard-coded orange-100 in dark mode', () => {
      ...
      const logoPlaceholder = document.querySelector('[class*="bg-orange-100"]');
      
      // Should not use hard-coded orange-100 background without dark variant
      expect(logoPlaceholder.className).not.toMatch(/bg-orange-100(?!\s+dark:)/);
      
      // Should use dark mode variant or semantic background
      expect(logoPlaceholder.className).toMatch(/dark:bg-orange-950\/30|bg-orange-100\s+dark:/);
    });
    ...
    test('should handle logo loading placeholder with proper dark mode styling', () => {
      ...
      const logoElements = document.querySelectorAll('[class*="bg-orange-100"]');
      
      logoElements.forEach(element => {
        // Should have dark mode variant for orange backgrounds
        if (element.className.includes('bg-orange-100')) {
          expect(element.className).toMatch(/dark:bg-orange-950\/30|dark:bg-orange/);
        }
      });
    });
  });
  ...
  describe('Text Colors Dark Mode', () => {
    test('should use semantic colors for organization name instead of hard-coded gray-900 in dark mode', () => {
      ...
      expect(organizationName.className).not.toMatch(/text-gray-900(?!\s+dark:)/);
      
      // Should use semantic color or dark mode variant
      expect(organizationName.className).toMatch(/text-foreground|text-card-title|dark:text-/);
    });
    ...
    test('should use semantic colors for location text instead of hard-coded gray-600 in dark mode', () => {
      ...
      // Should use semantic color or dark mode variant
      expect(locationText.className).toMatch(/text-muted-foreground|dark:text-/);
    });
    ...
    test('should use semantic colors for information sections instead of hard-coded gray-700 in dark mode', () => {
      ...
      [basedInText, dogsInText, shipsToText].forEach(element => {
        ...
        // Should use semantic color or dark mode variant
        expect(element.className).toMatch(/text-foreground|text-muted-foreground|dark:text-/);
      });
    });
  ...
  describe('Border and Background Dark Mode', () => {
    test('should use semantic border color instead of hard-coded gray-100 in dark mode', () => {
      ...
      const borderElements = document.querySelectorAll('[class*="border-gray-100"]');
    ...
    test('should use semantic background for dog thumbnails placeholder in dark mode', () => {
      ...
      const placeholderElements = document.querySelectorAll('[class*="bg-gray-200"]');
  ...
  describe('Button Colors Dark Mode', () => {
    test('should use semantic colors for outline button instead of hard-coded grays in dark mode', () => {
      ...
      const visitWebsiteButton = screen.getByRole('link', { name: /visit website/i });
      
      // Should not use hard-coded gray colors
      expect(visitWebsiteButton.className).not.toMatch(/text-gray-700(?!\s+dark:)/);
      expect(visitWebsiteButton.className).not.toMatch(/border-gray-300(?!\s+dark:)/);
      expect(visitWebsiteButton.className).not.toMatch(/hover:bg-gray-50(?!\s+dark:)/);
      
      // Should use semantic colors or dark mode variants
      expect(visitWebsiteButton.className).toMatch(/text-foreground|border-border|hover:bg-muted|dark:/);
    });
  ...
  describe('Focus States Dark Mode', () => {
    test('should maintain proper focus ring colors in dark mode', () => {
      ...
      expect(orgCard.className).toMatch(/focus:ring-orange-600|dark:focus:ring-orange/);

      // Check button focus states
      const visitButton = screen.getByRole('link', { name: /visit website/i });
      expect(visitButton.className).toMatch(/focus:ring-orange-600|dark:focus:ring-orange/);
    });
  });
  ...
  describe('Dark Mode Integration', () => {
    test('should work cohesively with card component styling in dark mode', () => {
      ...
      const allTextElements = card.querySelectorAll('[class*="text-gray"]:not([class*="dark:"])');
      
      // Count elements that don't have dark mode variants
      const elementsWithoutDarkMode = Array.from(allTextElements).filter(element => {
        return element.className.includes('text-gray') && !element.className.includes('dark:');
      });
    ...
    test('should handle all size variants properly in dark mode', () => {
      ...
      ['small', 'medium', 'large'].forEach(size => {
        ...
        expect(document.documentElement).toHaveClass('dark');
        expect(screen.getByText('Test Rescue Organization')).toBeInTheDocument();
