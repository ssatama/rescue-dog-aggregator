...
jest.mock('../../components/ui/LazyImage', () => {
  return function MockLazyImage({ alt, className, priority, enableProgressiveLoading, ...props }) {
...
describe('DogCard Component - Dark Mode Support', () => {
  ...
  describe('Text Colors Dark Mode', () => {
    test('should use semantic colors for age and gender text in dark mode', () => {
      ...
      expect(ageGenderRow.className).toMatch(/text-muted-foreground|dark:text-/);
      ...
      if (formattedAge) {
        ...
        expect(formattedAge.className).toMatch(/text-muted-foreground|dark:text-/);
      }
    });
    ...
    test('should use semantic colors for ships-to text in dark mode', () => {
      ...
      if (shipsToLabel) {
        ...
        // Should use semantic color or dark mode variant
        expect(shipsToLabel.className).toMatch(/text-muted-foreground|dark:text-/);
      }
    });
  });
  ...
  describe('Focus States Dark Mode', () => {
    test('should maintain proper focus ring colors in dark mode', () => {
      ...
      expect(imageLink.className).toMatch(/focus:ring-orange-600|dark:focus:ring-orange/);

      // Check name link focus states
      const nameLink = screen.getByTestId('dog-name').closest('a');
      expect(nameLink.className).toMatch(/focus:ring-orange-600|dark:focus:ring-orange/);
    });
  });
  ...
  describe('Dark Mode Integration', () => {
    test('should work cohesively with other dark mode components', () => {
      ...
      const grayElements = cardContent.querySelectorAll('[class*="text-gray"]:not([class*="dark:"])');
