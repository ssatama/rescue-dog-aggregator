...
describe('EmptyState Component - Dark Mode Support', () => {
  ...
  describe('Background Colors Dark Mode', () => {
    test('should use semantic background instead of hard-coded orange gradient in dark mode', () => {
      ...
      expect(emptyStateContainer.className).toMatch(/dark:from-orange-950|dark:from-orange-900/);
      expect(emptyStateContainer.className).toMatch(/dark:to-orange-900|dark:to-orange-800/);
    });
    ...
    test('should use semantic border color instead of hard-coded orange-200 in dark mode', () => {
      ...
      expect(emptyStateContainer.className).toMatch(/dark:border-orange-800|dark:border-orange-900/);
    });
  });
  ...
  describe('Text Colors Dark Mode', () => {
    ...
    test('should use semantic colors for description instead of hard-coded gray-700 in dark mode', () => {
      ...
      const descriptionElement = screen.getByText(/There are no items to display/);
      
      // Should not use hard-coded gray-700 color
      expect(descriptionElement.className).not.toMatch(/text-gray-700(?!\s+dark:)/);
      
      // Should use semantic color or dark mode variant
      expect(descriptionElement.className).toMatch(/text-muted-foreground|dark:text-/);
    });
  });
  ...
  describe('Icon Colors Dark Mode', () => {
    test('should use semantic colors for icons instead of hard-coded orange-400 in dark mode', () => {
      ...
      expect(iconClasses).toMatch(/text-orange-400.*dark:text-orange|dark:text-orange/);
    });

    test('should handle different variant icons properly in dark mode', () => {
      const variants = ['noDogsFiltered', 'noDogsOrganization', 'noOrganizations', 'default'];
      
      variants.forEach(variant => {
        const { unmount } = renderWithDarkTheme(<EmptyState variant={variant} />);
    ...
    test('should handle action button colors properly in dark mode', () => {
      ...
      renderWithDarkTheme(
        <EmptyState 
          ...
          onClearFilters={mockAction}
        />
      );

      const actionButton = screen.getByRole('button', { name: /clear all filters/i });
      ...
      expect(actionButton.className).toMatch(/hover:from-orange-700|hover:to-orange-800/);
      
      // Should have proper focus ring for dark mode
      expect(actionButton.className).toMatch(/focus:ring-orange-600|dark:focus:ring-orange/);
    });
  ...
  describe('Variant-Specific Dark Mode', () => {
    ...
    test('should handle noDogsOrganization variant properly in dark mode', () => {
      ...
      // Should display correct content
      expect(screen.getByText('No dogs available right now')).toBeInTheDocument();
      expect(screen.getByText(/This organization doesn't have/)).toBeInTheDocument();
      expect(screen.getByRole('button', { name: /explore other rescues/i })).toBeInTheDocument();
    ...
    test('should handle noOrganizations variant properly in dark mode', () => {
      ...
      expect(screen.getByText(/We couldn't find any rescue/)).toBeInTheDocument();
  ...
  describe('Dark Mode Integration', () => {
    test('should work cohesively with overall dark mode styling', () => {
      renderWithDarkTheme(<EmptyState variant="noDogsFiltered" onClearFilters={jest.fn()} />);
      ...
      const allGrayElements = emptyStateContainer.querySelectorAll('[class*="text-gray"]:not([class*="dark:"])');
