...
describe('Layout Components - Dark Mode Support', () => {
  ...
  describe('Header Component Dark Mode', () => {
    ...
    test('should have proper navigation link colors in dark mode', () => {
      ...
      links.forEach(link => {
        if (link.textContent && !link.textContent.includes('Skip to')) {
          ...
          expect(link.className).toMatch(/text-foreground|text-muted-foreground|dark:text-/);
        }
      });
    });
    ...
    test('should handle mobile menu styling in dark mode', () => {
      ...
      const mobileButton = screen.getByText(/open main menu/i).closest('button');
      expect(mobileButton).toBeInTheDocument();
      
      // Should have proper styling for dark mode
      expect(mobileButton.className).toMatch(/text-foreground|text-muted-foreground|dark:text-/);
    });
    ...
    test('should maintain theme toggle functionality in dark mode', () => {
      ...
      const moonIcons = document.querySelectorAll('svg[data-testid*="moon"], svg[stroke*="currentColor"]');
    ...
    test('should handle focus states properly in dark mode', () => {
      ...
      [...links, ...buttons].forEach(element => {
        if (element.getAttribute('href') !== '#main-content') { // Skip skip-link
  ...
  describe('Footer Component Dark Mode', () => {
    ...
    test('should have proper text colors in dark mode', () => {
      ...
      expect(footer.className).toMatch(/text-foreground|text-muted-foreground|text-card-foreground/);
      
      // Find text elements within footer
      const textElements = footer.querySelectorAll('p, h1, h2, h3, h4, h5, h6, span');
      textElements.forEach(element => {
        if (element.textContent.trim()) {
          expect(element.className).toMatch(/text-foreground|text-muted-foreground|text-card-foreground/);
        }
      });
    });
    ...
    test('should have proper link hover states in dark mode', () => {
      ...
      if (footerLinks.length > 0) {
        footerLinks.forEach(link => {
          ...
          expect(link.className).toMatch(/text-foreground|text-muted-foreground|dark:text-|hover:text-/);
        });
      }
    });
    ...
    test('should handle footer sections properly in dark mode', () => {
      ...
      expect(footer.className).toMatch(/border-border|border-muted|dark:border-/);
    });
  });
  ...
  describe('Layout Components Integration in Dark Mode', () => {
    ...
    test('should handle responsive layout in dark mode', () => {
      ...
      const mobileMenuButton = screen.getByText(/open main menu/i).closest('button');
