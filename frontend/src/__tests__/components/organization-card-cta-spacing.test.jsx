...
jest.mock('../../components/ui/LazyImage', () => {
  return function MockLazyImage({ alt, className, placeholder }) {
    return placeholder || <div className={className} data-testid="lazy-image">{alt}</div>;
  };
});
...
jest.mock('../../components/ui/SocialMediaLinks', () => {
  return function MockSocialMediaLinks({ socialMedia, className, size }) {
    return (
      <div className={className} data-testid="social-media-links" data-size={size}>
        {Object.keys(socialMedia).map(platform => (
          <a key={platform} href={socialMedia[platform]} data-testid={`social-${platform}`}>
            {platform}
          </a>
        ))}
      </div>
...
describe('OrganizationCard CTA Button Spacing Tests', () => {
  ...
  describe('Cross-Size Consistency', () => {
    test('all sizes maintain consistent button spacing', () => {
      ...
      sizes.forEach(size => {
        const { unmount } = render(<OrganizationCard organization={mockOrganization} size={size} />);
    ...
    test('all sizes meet minimum touch target requirements', () => {
      ...
      sizes.forEach(size => {
        ...
        const visitButton = screen.getByText('Visit Website');
        const buttons = screen.getAllByRole('button');
        const viewDogsButton = size === 'small' 
          ? buttons.find(button => button.tagName === 'BUTTON' && button.textContent.includes('Meet 25'))
          : buttons.find(button => 
              button.tagName === 'BUTTON' && button.textContent.includes('View') && button.textContent.includes('25') && button.textContent.includes('Dogs')
            );
