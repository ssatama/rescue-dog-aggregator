...
jest.mock('../../utils/countries', () => {
  ...
  return {
    ...
    formatBasedIn: jest.fn((country, city, mobile) => {
      ...
      return mobile ? countryName : (city ? `${city}, ${countryName}` : countryName);
    }),
    formatServiceRegions: jest.fn((regions, showFlags, mobile) => {
      return regions.map(region => originalModule.getCountryName(region)).join(', ');
    }),
    formatShipsToList: jest.fn((countries, limit) => {
      const names = countries.map(country => originalModule.getCountryName(country));
      return countries.length <= limit ? names.join(', ') : `${names.slice(0, limit).join(', ')} +${countries.length - limit} more`;
    })
  };
});
...
describe('Pets in Turkey Fix', () => {
  describe('Enhanced Country Name Handling', () => {
    test('normalizeCountryCode handles full country name "Turkey"', () => {
      ...
      const { normalizeCountryCode } = jest.requireActual('../../utils/countries');
    ...
    test('complete Turkey input processing flow', () => {
      const { normalizeCountryCode, getCountryName } = jest.requireActual('../../utils/countries');
  ...
  describe('CountryFlag with Full Country Names', () => {
    test('displays Turkey flag when given full country name "Turkey"', () => {
      render(<CountryFlag countryCode="Turkey" countryName="Turkey" size="small" />);
      
      const flagImage = screen.queryByRole('img');
      expect(flagImage).toBeInTheDocument();
      expect(flagImage).toHaveAttribute('src', 'https://flagcdn.com/20x15/tr.png');
      expect(flagImage).toHaveAttribute('alt', 'Turkey flag');
    });

    test('displays Germany flag when given full country name "Germany"', () => {
      render(<CountryFlag countryCode="Germany" countryName="Germany" size="small" />);
      
      const flagImage = screen.queryByRole('img');
      expect(flagImage).toBeInTheDocument();
      expect(flagImage).toHaveAttribute('src', 'https://flagcdn.com/20x15/de.png');
    });
