...
const MockOrganizationCard = ({ organization, size = 'medium' }) => {
  return (
    <div data-testid={`org-card-${organization.config_id}`} className={`org-card size-${size}`}>
...
describe('Organization Logo R2 Migration', () => {
  ...
  describe('Organization Card Logo Display', () => {
    it('should render PIT logo without errors', async () => {
      const pitOrg = mockOrganizations.find(org => org.config_id === 'pets-in-turkey');
    ...
    it('should render Tierschutzverein logo without errors', async () => {
      const tsOrg = mockOrganizations.find(org => org.config_id === 'tierschutzverein-europa');
    ...
    it('should render REAN logo without errors', async () => {
      ...
      expect(logo).toHaveAttribute('alt', 'REAN (Rescuing European Animals in Need) logo');
