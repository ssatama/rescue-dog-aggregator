    ...
    return (
      ...
      >
        ...
        <div className="organization-stats">
          ...
          <p className="location">{organization.city}, {organization.country}</p>
        </div>
...
describe('Organizations Display Integration', () => {
  beforeEach(() => {
    ...
    mockGetStatistics.mockImplementation(() => Promise.resolve({
      total_dogs: 450,
      total_organizations: 7,
      countries: ['DE', 'TR', 'UK', 'US', 'FR', 'ES', 'IT'],
      organizations: [
  ...
  test('renders complete TrustSection with organizations', async () => {
    ...
    expect(screen.getByText('Dogs available from these organizations:')).toBeInTheDocument();
  ...
  test('displays all 7 organizations with proper data mapping', async () => {
    ...
    expect(screen.getByText('Tierschutzverein Europa e.V.')).toBeInTheDocument();
  ...
  test('correctly maps API fields to OrganizationCard props', async () => {
    ...
    const reanCard = organizationCards.find(card => card.getAttribute('data-org-id') === '1');
  ...
  test('renders CTA buttons for all organizations', async () => {
    ...
    const ctaButtons = screen.getAllByRole('button', { name: /View \d+ dogs from/i });
  ...
  test('handles organization logo rendering', async () => {
    ...
    expect(screen.getByAltText('REAN logo')).toHaveAttribute('src', 'https://example.com/logo1.jpg');
    expect(screen.getByAltText('Pets in Turkey logo')).toHaveAttribute('src', 'https://example.com/logo2.jpg');
  });
  ...
  test('handles error states gracefully', async () => {
    ...
    expect(screen.getByText('Unable to load statistics. Please try again later.')).toBeInTheDocument();
  });

  test('handles loading states properly', async () => {
    // Mock delayed response
    const mockGetStatistics = require('../../services/animalsService').getStatistics;
    mockGetStatistics.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
