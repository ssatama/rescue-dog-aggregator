...
describe("Metadata Validation Integration Tests", () => {
  ...
  describe("OpenGraph Type Validation", () => {
    test("should use valid OpenGraph types for all pages", async () => {
      ...
      const dogMetadata = await generateDogMetadata({
        params: { slug: "test-dog-test-breed-1" },
      });
      ...
      const orgMetadata = await generateOrgMetadata({
        params: { slug: "test-organization-1" },
      });
