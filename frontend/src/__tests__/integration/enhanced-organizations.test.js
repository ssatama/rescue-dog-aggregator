...
jest.mock('../../utils/countryUtils', () => ({
  ...
  formatServiceRegions: jest.fn((regions) => regions.map(r => `🇹🇷 ${r}`).join(', ')),
  formatShipsTo: jest.fn((countries) => 
    countries.length <= 3 ? countries.map(c => `🇩🇪 ${c}`).join(' ') : `🇩🇪 ${countries[0]} +${countries.length - 1} more`
  )
}));
...
describe('Enhanced Organizations Integration', () => {
  const mockEnhancedOrganizations = [
    ...
    },
    {
      ...
      new_this_week: 0,
      recent_dogs: [
        { id: 4, name: 'Charlie', thumbnail_url: 'https://example.com/dog4.jpg' }
      ],
      social_media: {
  ...
  describe('Successful Data Loading', () => {
    test('loads and displays enhanced organization cards', async () => {
      organizationsService.getEnhancedOrganizations.mockResolvedValueOnce(mockEnhancedOrganizations);
    ...
    test('displays recent dog previews correctly', async () => {
      ...
      expect(screen.getByText(/Buddy, Max, Luna and 30 more looking for homes/)).toBeInTheDocument();
  ...
  describe('Error Handling', () => {
    test('displays error message when API fails', async () => {
      ...
      organizationsService.getEnhancedOrganizations.mockRejectedValueOnce(mockError);
    ...
    test('retry functionality works correctly', async () => {
      ...
      expect(screen.queryByText('There was an error loading organizations. Please try again later.')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Card Interactions', () => {
    ...
    test('visit website buttons work correctly', async () => {
      ...
      expect(visitButtons[0].closest('a')).toHaveAttribute('href', 'https://petsinturkey.org');
      expect(visitButtons[1].closest('a')).toHaveAttribute('href', 'https://rean.org');
      expect(visitButtons[2].closest('a')).toHaveAttribute('href', 'https://tierschutzverein-europa.de');
    });
  ...
  describe('Performance and Loading States', () => {
    test('shows proper loading state during data fetch', async () => {
      ...
      organizationsService.getEnhancedOrganizations.mockReturnValueOnce(controlledPromise);
    ...
    test('handles slow API responses gracefully', async () => {
      ...
      organizationsService.getEnhancedOrganizations.mockReturnValueOnce(slowPromise);
  ...
  describe('Accessibility', () => {
    test('page has proper heading structure', async () => {
      ...
      // Check main heading
      expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Rescue Organizations');
