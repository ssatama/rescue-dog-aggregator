...
describe('Dev Server Startup Validation', () => {
  describe('Development Server', () => {
    test('should start dev server successfully without hanging', async () => {
      ...
      try {
        ...
        while (!startupSuccess && (Date.now() - startTime) < STARTUP_TIMEOUT) {
          ...
          if (stdoutOutput.includes(READY_MESSAGE) || stdoutOutput.includes('- Local:')) {
            startupSuccess = true;
            break;
          }
        ...
        if (!startupSuccess) {
          throw new Error(`Dev server failed to start within ${STARTUP_TIMEOUT}ms.\n\nSTDOUT:\n${stdoutOutput}\n\nSTDERR:\n${errorOutput}`);
        }
        ...
        for (const error of criticalErrors) {
          if (stdoutOutput.includes(error) || errorOutput.includes(error)) {
            throw new Error(`Critical error detected during startup: ${error}\n\nOutput:\n${stdoutOutput}\n${errorOutput}`);
          }
        }
    ...
    }, STARTUP_TIMEOUT + 5000); // Add buffer to test timeout
