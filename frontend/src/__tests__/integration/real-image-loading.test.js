...
describe("Real Image Loading Integration", () => {
  ...
  describe("Environment Configuration Validation", () => {
    test("R2 custom domain is configured", () => {
      const r2Domain =
        process.env.NEXT_PUBLIC_R2_CUSTOM_DOMAIN || R2_CUSTOM_DOMAIN;
    ...
    test("API URL is configured for local development", () => {
      ...
      expect(["http://localhost:8000", "http://localhost:3000"]).toContain(
        apiUrl,
      );
    });
  });
    ...
    test("transformed URLs maintain valid R2 format", () => {
      ...
      transformations.forEach((transformed) => {
        ...
        expect(transformed).toMatch(
          new RegExp(`^https:\\/\\/${R2_CUSTOM_DOMAIN}\\/cdn-cgi\\/image\\/`),
        );
