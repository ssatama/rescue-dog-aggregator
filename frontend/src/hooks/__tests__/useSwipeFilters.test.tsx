...
describe('useSwipeFilters', () => {
  ...
  describe('Query String Generation', () => {
    it('should generate query string for API calls', () => {
      ...
      expect(result.current.toQueryString()).toBe('country=Germany&size=small&size=medium');
    });
  ...
  describe('Onboarding Status', () => {
    ...
    it('should mark onboarding complete', () => {
      ...
      act(() => {
        result.current.completeOnboarding({ country: 'Germany', sizes: ['small'] });
      });
      
      expect(localStorage.getItem('swipeOnboardingComplete')).toBe('true');
      expect(result.current.filters).toEqual({ country: 'Germany', sizes: ['small'] });
