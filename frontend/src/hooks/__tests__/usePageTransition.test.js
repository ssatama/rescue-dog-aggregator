...
function TestComponent({ delay = 0, duration = 300 }) {
  ...
  return (
    <div data-testid="test-component" className={className}>
      <span data-testid="visibility-indicator">{isVisible ? 'visible' : 'hidden'}</span>
    </div>
  );
}
...
describe('usePageTransition', () => {
  ...
  describe('PageTransition component', () => {
    ...
    it('applies custom delay and duration', async () => {
      const { container } = render(
        <PageTransition delay={300} duration={500} data-testid="page-transition">
          <div>Content</div>
        </PageTransition>
      );
    ...
    it('merges custom className', () => {
      const { container } = render(
        <PageTransition className="custom-class bg-red-500" data-testid="page-transition">
          <div>Content</div>
        </PageTransition>
      );
