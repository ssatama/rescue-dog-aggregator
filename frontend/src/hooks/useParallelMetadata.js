...
export function useParallelMetadata() {
  ...
  const locationCountriesCache = useMetadataCache('location-countries', getLocationCountries);
  const availableCountriesCache = useMetadataCache('available-countries', getAvailableCountries);
  const organizationsCache = useMetadataCache('organizations', getOrganizations);
  ...
  const fetchAllMetadata = useCallback(async () => {
    ...
    try {
      ...
      const processedBreeds = breeds ? ['Any breed', ...breeds.filter(b => b !== 'Any breed')] : ['Any breed'];
      const processedLocationCountries = locationCountries ? ['Any country', ...locationCountries] : ['Any country'];
      const processedAvailableCountries = availableCountries ? ['Any country', ...availableCountries] : ['Any country'];
  ...
  }, [breedsCache, locationCountriesCache, availableCountriesCache, organizationsCache]);
