 ...
 *
 * Key breakpoint behavior with Tailwind CSS (lg: 1024px):
 * - < 1024px = Mobile layout (mobile filter button visible)
 * - ≥ 1024px = Desktop layout (desktop filter sidebar, mobile button hidden)
 */
const DEVICES = [
  ...
  /* {
    ...
    // CSS Viewport: 384×824 (mobile layout < 1024px)
    width: 384,
    height: 824,
  ...
  // === TABLETS (Mixed based on orientation) ===
  {
    ...
    // CSS Viewport: 744×1026 portrait (mobile layout < 1024px)
    width: 744,
    height: 1026,
    deviceScaleFactor: 2,
    userAgent:
  ...
  },
  {
    ...
    // CSS Viewport: 820×1180 portrait (mobile layout < 1024px)
    width: 820,
    height: 1180,
    deviceScaleFactor: 2,
    userAgent:
  ...
  /* {
  {
    ...
    // CSS Viewport: 1024×1366 portrait (desktop layout ≥ 1024px)
    width: 1024,
    height: 1366,
    deviceScaleFactor: 2,
  ...
  // === DESKTOP (≥ 1024px) ===
  {
    ...
    // CSS Viewport: 1280×715 (desktop layout ≥ 1024px)
    width: 1280,
    height: 715,
    deviceScaleFactor: 2,
    userAgent:
      "Mozilla/5.0 (Macintosh; Intel Mac OS X 15_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36",
    isDesktop: true,
  },
];
...
const PAGES = [
  ...
  {
    name: "dog-detail",
    url: "http://localhost:3000/dogs/saga-mixed-breed-1835",
  },
    ...
    setup: async (page) => {
      ...
      try {
        ...
        if (favoriteButtons.length === 0) {
          console.log(
            `    ⚠️  No favorite buttons found, trying alternative selectors...`
          );
          ...
          for (let i = 0; i < Math.min(3, heartButtons.length); i++) {
            try {
              const ariaLabel =
                await heartButtons[i].getAttribute("aria-label");
          ...
          console.log(
            `    📌 Found ${favoriteButtons.length} favorite buttons, clicking ${numToAdd}`
          );
          ...
          for (let i = 0; i < numToAdd; i++) {
            try {
              ...
              const ariaLabel =
                await favoriteButtons[i].getAttribute("aria-label");
...
async function verifyViewport(page, expectedWidth, expectedHeight) {
  ...
  if (viewport.width !== expectedWidth || viewport.height !== expectedHeight) {
    throw new Error(
      `Viewport mismatch! Expected ${expectedWidth}x${expectedHeight}, got ${viewport.width}x${viewport.height}`
    );
  }
...
async function waitForPageReady(page, pageConfig) {
  ...
  try {
    ...
    console.log(
      `    ⚠️  Content check timeout for ${pageConfig.name}, continuing anyway...`
    );
...
async function triggerLazyLoading(page) {
  ...
  const isAtTop = await page.evaluate(async () => {
    ...
    while (currentPosition > 0) {
      ...
      const loadingImages = Array.from(document.querySelectorAll("img")).filter(
        (img) => !img.complete || img.naturalHeight === 0
      );

      if (
        loadingImages.length > 0 &&
        loadingImages.length !== loadingImageCount
      ) {
...
async function captureScreenshots() {
    ...
    for (const device of DEVICES) {
      ...
      for (const pageConfig of PAGES) {
        ...
        try {
          ...
          totalCaptured++;
          console.log(
            `    ✅ Saved: ${filename} (${totalCaptured}/${totalScreenshots})`
          );
          ...
          if (isMobile && pageConfig.mobileStates) {
            console.log(
              `    📱 Capturing ${pageConfig.mobileStates.length} mobile state(s)...`
            );
          ...
          console.error(
            `    ❌ Error capturing ${pageConfig.name}:`,
            error.message
          );
  ...
  console.log(
    `📊 Total screenshots captured: ${totalCaptured}/${totalScreenshots}`
  );
}
