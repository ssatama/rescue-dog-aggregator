...
def age_category_condition(category: str) -> str | None:
    ...
    hides it (#494). A half-populated row (one bound set) is not treated
    ...
    return f"(({' AND '.join(clauses)}) OR {NO_RECORDED_AGE})"
...
class AnimalService:
    ...
    def _build_single_animal_response(self, row: dict) -> Animal:
        ...
        animal_dict = {
            ...
            "primary_image_url": _normalize_url(row.get("primary_image_url")),
            "adoption_url": _normalize_url(row.get("adoption_url")),
