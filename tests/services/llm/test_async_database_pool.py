...
from services.llm.async_database_pool import AsyncDatabasePool, ConnectionContext, PoolConfig
...
class TestAsyncDatabasePool:
    ...
    def pool_config(self):
        ...
        return PoolConfig(host="localhost", database="test_rescue_dogs", user="test_user", password="", min_connections=2, max_connections=10, timeout=5.0)
    ...
    async def test_pool_execute_query(self, pool_config):
        ...
        with patch("asyncpg.create_pool", new_callable=AsyncMock) as mock_create:
            mock_pool = AsyncMock()
            mock_connection = AsyncMock()
            mock_connection.fetch.return_value = [{"id": 1, "name": "Buddy"}, {"id": 2, "name": "Max"}]
    ...
    async def test_pool_retry_on_connection_error(self, pool_config):
        ...
        with patch("asyncpg.create_pool", new_callable=AsyncMock) as mock_create:
            ...
            mock_connection.fetch.side_effect = [asyncpg.exceptions.ConnectionDoesNotExistError("Connection lost"), [{"id": 1, "name": "Buddy"}]]
...
class TestPoolConfig:
    ...
    def test_config_from_environment(self):
        ...
        with patch.dict("os.environ", {"DB_HOST": "test-host", "DB_NAME": "test-db", "DB_USER": "test-user", "DB_PASSWORD": "test-pass", "DB_POOL_MIN": "5", "DB_POOL_MAX": "20"}):
    ...
    def test_config_validation(self):
        ...
        with pytest.raises(ValueError, match="min_connections.*max_connections"):
            PoolConfig(host="localhost", database="test", user="user", password="", min_connections=20, max_connections=10)

        # Negative values should raise
        with pytest.raises(ValueError, match="must be positive"):
            PoolConfig(host="localhost", database="test", user="user", password="", min_connections=-1, max_connections=10)
