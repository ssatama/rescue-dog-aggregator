...
from services.llm.dog_profiler_service import BatchProfileResult, DogProfilerService, ProfileRequest, ProfileResult
...
class TestDogProfilerService:
    ...
    def mock_normalizer(self):
        ...
        mock = Mock()
        mock.normalize.return_value = {"description": "Normalized description", "tagline": "Normalized tagline", "energy_level": "medium"}
        return mock
    ...
    def mock_config_loader(self):
        ...
        mock = Mock()
        mock.load_config.return_value = Mock(organization_id=11, prompt_file="test.yaml", model_preference="google/gemini-2.5-flash")
        mock.load_prompt_template.return_value = Mock(system_prompt="System prompt", user_prompt="User prompt: {dog_data}", examples=[])
        return mock
    ...
    def service(self, mock_pool, mock_llm_client, mock_normalizer, mock_config_loader):
        ...
        with patch("services.llm.dog_profiler_service.ProfileNormalizer", return_value=mock_normalizer):
            with patch("services.llm.dog_profiler_service.get_config_loader", return_value=mock_config_loader):
                service = DogProfilerService(pool=mock_pool, llm_client=mock_llm_client)
                return service
    ...
    async def test_profile_single_dog(self, service, mock_llm_client):
        ...
        # Setup
        dog_data = {"id": 1, "name": "Buddy", "properties": {"age": "2 years", "breed": "German Shepherd"}}

        mock_llm_client.generate.return_value = {"description": "Friendly dog", "tagline": "Your best friend", "energy_level": "high"}

        # Execute
        request = ProfileRequest(dog_id=1, dog_data=dog_data, organization_id=11)
    ...
    async def test_profile_batch(self, service, mock_llm_client):
        ...
        # Setup
        dogs = [{"id": 1, "name": "Buddy", "properties": {}}, {"id": 2, "name": "Max", "properties": {}}, {"id": 3, "name": "Lucy", "properties": {}}]

        mock_llm_client.generate.return_value = {"description": "Test description", "tagline": "Test tagline"}
    ...
    async def test_profile_batch_with_partial_failure(self, service, mock_llm_client):
        ...
        mock_llm_client.generate.side_effect = [{"description": "Success", "tagline": "Good dog"}, Exception("API error")]
    ...
    async def test_save_profile_to_database(self, service, mock_pool):
        ...
        # Setup
        profile = {"description": "Test description", "tagline": "Test tagline", "profiled_at": datetime.now().isoformat()}
    ...
    async def test_get_unprofiled_dogs(self, service, mock_pool):
        ...
        # Execute
        dogs = await service.get_unprofiled_dogs(organization_id=11, limit=10)
    ...
    async def test_profile_with_retry(self, service, mock_llm_client):
        ...
        mock_llm_client.generate.side_effect = [Exception("Temporary error"), {"description": "Success", "tagline": "Good dog"}]
    ...
    async def test_profile_with_validation(self, service, mock_llm_client, mock_normalizer):
        ...
        mock_llm_client.generate.return_value = {"description": "Too short", "tagline": "Test"}  # Should fail validation

        mock_normalizer.normalize.return_value = {"description": "Too short", "tagline": "Test"}
...
class TestProfileModels:
    ...
    def test_profile_request(self):
        ...
        request = ProfileRequest(dog_id=1, dog_data={"name": "Buddy"}, organization_id=11, force_regenerate=True)
    ...
    def test_profile_result(self):
        ...
        result = ProfileResult(dog_id=1, success=True, profile={"description": "Test"}, processing_time_ms=1000, model_used="gemini-2.5-flash")
    ...
    def test_batch_profile_result(self):
        ...
        result = BatchProfileResult(organization_id=11, success=8, failed=2, total=10, results=[], processing_time_ms=10000)
