...
class TestIndexSync:
    ...
    def test_sync_indexes_creates_missing(self, mock_railway_session, mock_get_local, mock_get_railway):
        ...
        assert any('CREATE INDEX idx_animals_new' in str(call) for call in calls)
        assert not any('CONCURRENTLY' in str(call) for call in calls)
    ...
    def test_sync_indexes_drops_extra(self, mock_railway_session, mock_get_local, mock_get_railway):
        ...
        calls = mock_session.execute.call_args_list
        assert any('DROP INDEX IF EXISTS idx_animals_old' in str(call) for call in calls)
    ...
    def test_sync_indexes_preserves_unique_constraints(self, mock_railway_session, mock_get_local, mock_get_railway):
        ...
        calls = mock_session.execute.call_args_list
        assert any('DROP INDEX IF EXISTS idx_animals_old' in str(call) for call in calls)
        assert not any('animals_pkey' in str(call) for call in calls)
        assert not any('animals_slug_unique' in str(call) for call in calls)
        assert not any('animals_external_key' in str(call) for call in calls)
