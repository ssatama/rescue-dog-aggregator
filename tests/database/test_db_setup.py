...
class TestDatabaseSetup:
    ...
    @patch('database.db_setup.DB_CONFIG',
           {'host': 'localhost',
            'user': 'test',
            'database': 'test_db',
            'password': 'test_pass'})
    ...
    @patch('database.db_setup.DB_CONFIG',
           {'host': 'localhost', 'user': 'test', 'database': 'test_db', 'password': ''})
    @patch('database.db_setup.psycopg2.connect')
    ...
    @patch('builtins.open',
           mock_open(read_data="CREATE TABLE test (id SERIAL PRIMARY KEY);"))
    ...
    def test_create_tables_duplicate_table_handling(self):
        ...
        mock_cursor = MagicMock()
        mock_conn.cursor.return_value = mock_cursor
        mock_cursor.execute.side_effect = errors.DuplicateTable(
    ...
    def test_initialize_database_success(
            self, mock_connect, mock_create_tables):
    ...
    @patch('database.db_setup.create_tables')
    @patch('database.db_setup.connect_to_database')
    def test_initialize_database_connection_failure(
            self, mock_connect, mock_create_tables):
    ...
    @patch('database.db_setup.create_tables')
    @patch('database.db_setup.connect_to_database')
    def test_initialize_database_schema_failure(
            self, mock_connect, mock_create_tables):
    ...
    def test_add_organization_success(self):
        ...
        mock_cursor.execute.assert_any_call(
            "SELECT id FROM organizations WHERE name = %s", ("Test Org",))
    ...
    def test_add_organization_already_exists(self):
        ...
        mock_cursor.fetchone.return_value = (
            123,)  # Organization exists with ID 123
    ...
    def test_add_organization_database_error(self):
        ...
        mock_cursor.execute.side_effect = [
            None, psycopg2.Error("Database error")]
    ...
    @patch('database.db_setup.add_organization')
    @patch('database.db_setup.initialize_database')
    def test_setup_initial_data_initialization_failure(
            self, mock_initialize, mock_add_org):
    ...
    @patch('database.db_setup.add_organization')
    @patch('database.db_setup.initialize_database')
    def test_setup_initial_data_organization_failure(
            self, mock_initialize, mock_add_org):
    ...
    @patch('database.db_setup.add_organization')
    @patch('database.db_setup.initialize_database')
    def test_setup_initial_data_exception_handling(
            self, mock_initialize, mock_add_org):
