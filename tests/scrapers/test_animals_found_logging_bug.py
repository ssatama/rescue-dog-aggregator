...
class TestAnimalsFoundLoggingBug:
    ...
    def test_scraper_animals_found_attribute_preserved_after_run(self):
        ...
        runner = SecureConfigScraperRunner(
            ...
            scraper_loader=None,  # Will be mocked
            sync_service=None     # Will be mocked
        )
        
        # Mock the scraper loading to return our mock scraper
        with patch.object(runner, 'load_scraper_safely', return_value=mock_scraper), \
             patch.object(runner, 'ensure_organization_synced', return_value=1):
    ...
    def test_multiple_scrapers_preserve_animals_found_independently(self):
        ...
        with patch.object(runner, 'load_scraper_safely', side_effect=mock_load_scraper):
            ...
            for i, scraper_result in enumerate(result.results):
                ...
                assert scraper_result.animals_found == expected_animals[i], \
                    f"Scraper {i}: expected {expected_animals[i]} animals, got {scraper_result.animals_found}"
