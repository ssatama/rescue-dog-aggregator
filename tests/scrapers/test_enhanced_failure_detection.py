...
class TestEnhancedFailureDetection:
    ...
    def mock_scraper(self):
        ...
        scraper.detect_partial_failure = BaseScraper.detect_partial_failure.__get__(
            scraper)
        scraper.detect_catastrophic_failure = BaseScraper.detect_catastrophic_failure.__get__(
            scraper)
    ...
    def test_detect_extremely_low_count_as_catastrophic(self, mock_scraper):
        ...
        result = mock_scraper.detect_catastrophic_failure(
            1, absolute_minimum=3)
        assert result is True

        result = mock_scraper.detect_catastrophic_failure(
            2, absolute_minimum=3)
        assert result is True
    ...
    def test_database_error_during_failure_detection(self, mock_scraper):
        ...
        # Mock database error
        mock_scraper.conn.cursor.side_effect = Exception(
    ...
    def test_combined_catastrophic_and_partial_failure_check(
            self, mock_scraper):
        ...
        cursor_mock = Mock()
        cursor_mock.fetchone.return_value = (
            50.0, 5)  # 50.0 average, 5 historical scrapes
        mock_scraper.conn.cursor.return_value = cursor_mock
        ...
        result = mock_scraper.detect_scraper_failure(
            2, absolute_minimum=5)  # Below absolute minimum
        assert result is True

        result = mock_scraper.detect_scraper_failure(
            20, absolute_minimum=5)  # Below 50% threshold
        assert result is True

        result = mock_scraper.detect_scraper_failure(
            30, absolute_minimum=5)  # Normal
        assert result is False
...
class TestFailureLoggingAndReporting:
    ...
    def test_catastrophic_failure_logging_includes_context(self, mock_scraper):
        ...
        assert "organization_id" in log_message.lower() or str(
            mock_scraper.organization_id) in log_message
    ...
    def test_partial_failure_logging_includes_thresholds(self, mock_scraper):
        ...
        cursor_mock = Mock()
        cursor_mock.fetchone.return_value = (
            20.0, 5)  # 20.0 average, 5 historical scrapes
        mock_scraper.conn.cursor.return_value = cursor_mock
