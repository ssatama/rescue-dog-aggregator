...
class TestBaseScraperBatchUploads:
    ...
    def mock_services(self):
        ...
        with patch("scrapers.base_scraper.create_default_sync_service") as mock_sync, \
    ...
    def test_batch_upload_for_large_dataset(self, mock_services):
        ...
        class TestScraper(BaseScraper):
            def collect_data(self):
                return [
                    {"name": f"Dog {i}", "external_id": f"test-{i}", "primary_image_url": f"https://example.com/dog{i}.jpg"}
                    for i in range(15)
                ]
    ...
    def test_batch_upload_with_no_images(self, mock_services):
        ...
        class TestScraper(BaseScraper):
            def collect_data(self):
                return [
                    {"name": "Dog 1", "external_id": "test-1"},  # No image
                    {"name": "Dog 2", "external_id": "test-2", "primary_image_url": "https://example.com/dog2.jpg"}
                ]
