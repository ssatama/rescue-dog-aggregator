...
class TestSwipePerformance:
    ...
    def test_swipe_query_performance_under_100ms(self):
        mock_result = Mock()
        mock_result.fetchall.return_value = [(i, f"Dog{i}", {"quality_score": 0.8}, datetime.now(), f"Org{i}", "Germany") for i in range(20)]
        self.mock_db.execute.return_value = mock_result

        start_time = time.time()
        result = self.swipe_queries.get_swipe_queue(country="Germany", size=None, limit=20, offset=0, excluded_ids=[])
    ...
    def test_batch_loading_reduces_queries(self):
        mock_result = Mock()
        mock_result.fetchall.return_value = [(i, f"Dog{i}", {"quality_score": 0.8}, datetime.now(), f"Org{i}", "Germany") for i in range(15)]
        self.mock_db.execute.return_value = mock_result
    ...
    def test_index_usage_for_filters(self):
        mock_explain = Mock()
        mock_explain.scalars.return_value.all.return_value = ["Index Scan using idx_animals_country_quality"]

        with patch.object(self.swipe_queries, "explain_query") as mock_explain_method:
            mock_explain_method.return_value = ["Index Scan"]

            query_plan = self.swipe_queries.explain_query(country="Germany", size="Medium")
    ...
    def test_efficient_filtering_with_multiple_sizes(self):
        mock_result = Mock()
        mock_result.fetchall.return_value = [(i, f"Dog{i}", {"size": "Medium", "quality_score": 0.8}, datetime.now(), f"Org{i}", "Germany") for i in range(10)]
        self.mock_db.execute.return_value = mock_result

        result = self.swipe_queries.get_swipe_queue(country="Germany", size=["Small", "Medium", "Large"], limit=20, offset=0)
    ...
    def test_pagination_performance_with_large_offset(self):
        ...
        start_time = time.time()
        result = self.swipe_queries.get_swipe_queue(country="Germany", limit=20, offset=1000)
        execution_time = (time.time() - start_time) * 1000
