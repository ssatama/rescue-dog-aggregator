...
class TestSwipeEndpoint:
    ...
    def test_swipe_endpoint_returns_initial_stack(self, client: TestClient):
        """Test that /api/dogs/swipe returns initial stack of dogs"""
    ...
    def test_swipe_endpoint_filters_by_size(self, client: TestClient):
        ...
        # Test filtering by small dogs
        small_response = client.get("/api/dogs/swipe?size=small&limit=50")
    ...
    def test_swipe_endpoint_excludes_swiped_dogs(self, client: TestClient):
        """Test that endpoint excludes already swiped dogs from results"""
        # Get initial dogs
        ...
        if len(initial_data['dogs']) > 0:
            ...
            excluded_ids = [dog['id'] for dog in initial_data['dogs'][:3]]
            excluded_str = ','.join(map(str, excluded_ids))
            
            # Request with excluded IDs
            filtered_response = client.get(f"/api/dogs/swipe?excluded={excluded_str}&limit=50")
            assert filtered_response.status_code == 200
            filtered_data = filtered_response.json()
            
            # Verify no excluded IDs are in the results
            returned_ids = [dog['id'] for dog in filtered_data['dogs']]
            for excluded_id in excluded_ids:
                assert excluded_id not in returned_ids
    
    def test_swipe_endpoint_only_returns_quality_dogs(self, client: TestClient):
        """Test that endpoint only returns dogs with quality_score > 0.7"""
        response = client.get("/api/dogs/swipe?limit=50")
        assert response.status_code == 200
        data = response.json()
