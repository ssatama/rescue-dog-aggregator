...
def analyze_properties(properties_data):
    ...
    for animal_id, props in properties_data:
        ...
        for key in ['color', 'coat', 'temperament', 'good_with_kids', 'good_with_cats', 'good_with_dogs']:
...
def create_standardization_templates(breed_data, size_data, age_data):
    ...
    with open(os.path.join('data', 'audit', 'breed_standardization.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
    ...
    with open(os.path.join('data', 'audit', 'size_standardization.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['original_size', 'count', 'standardized_size', 'weight_range_kg', 'height_range_cm', 'notes'])
        
        for size, count in size_data:
            writer.writerow([size, count, '', '', '', ''])
    
    # Age standardization template
    with open(os.path.join('data', 'audit', 'age_standardization.csv'), 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['original_age_text', 'count', 'age_category', 'min_months', 'max_months', 'notes'])
