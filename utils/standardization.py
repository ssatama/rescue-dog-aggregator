...
BREED_MAPPING = {
    ...
    "american staffordshire terrier": ("American Staffordshire Terrier", "Terrier", "Medium"),
...
BREED_INDICATORS = [
    "mix", "cross", "blend", "hybrid", "combo", "mixed", 
    "shepherd", "retriever", "terrier", "hound", "spaniel", "poodle"
]
...
def parse_age_text(age_text: str) -> Tuple[Optional[str], Optional[int], Optional[int]]:
    ...
    years_match = re.search(r'(\d+(?:[.,]\d+)?)\s*(?:years?|y(?:rs?)?(?:\/o)?|yo|jahr)', age_text)
    if years_match:
